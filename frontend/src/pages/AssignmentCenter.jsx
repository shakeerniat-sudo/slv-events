        ...
        <div className="grid grid-cols-1 gap-6">
          <AnimatePresence mode="popLayout">
            {filteredEvents.map((event, idx) => {
              ...
              const stageVal = event.workflow_stage || 0;
              ...
              return (
                <motion.div
                  ...
                  className={`bg-white rounded-2xl border ${borderClass} shadow-sm hover:shadow-md transition-all duration-300 p-6 flex flex-col gap-6 stagger-${(idx % 6) + 1}`}
                >
                  {/* Card Header & Details */}
                  <div className="flex flex-col lg:flex-row justify-between gap-4 pb-4 border-b border-slate-100">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        <h3 className="text-base font-bold text-slate-800 tracking-tight font-outfit">{event.name}</h3>
                        ...
                        <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md font-bold uppercase font-outfit">
                          Stage {stageVal}: {
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-2 gap-x-4 text-xs font-semibold text-slate-500">
                        <div className="flex flex-col gap-1">
                          <span className="flex items-center gap-1.5">
                            <UserCheck className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                            <span>Client: <strong className="text-slate-700">{event.client_name}</strong></span>
                          </span>
                          <div className="pl-5 space-y-0.5 text-[10px] text-slate-400 font-medium">
                            {event.client_phone && <span className="flex items-center gap-1"><Phone className="w-3.5 h-3.5" /> {event.client_phone}</span>}
                            {event.client_email && <span className="flex items-center gap-1"><Mail className="w-3.5 h-3.5" /> {event.client_email}</span>}
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <span className="flex items-center gap-1.5">
                            ...
                            <span>Date: <strong className="text-slate-700">{new Date(event.event_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</strong></span>
                          </span>
                          <span className="flex items-center gap-1.5 pl-5 text-[10px] text-slate-400">
                            <Clock className="w-3.5 h-3.5 text-slate-400" />
                            <span>Time: <strong className="text-slate-600">{event.event_time || '10:00 AM - 04:00 PM'}</strong></span>
                          </span>
                        </div>
                        ...
                        <div className="space-y-1">
                          <span className="flex items-center gap-1.5">
                            ...
                            <span className="truncate" title={event.venue}>Venue: <strong className="text-slate-700">{event.venue}</strong></span>
                          </span>
                        </div>
                      </div>
                      ...
                      {event.status?.toLowerCase() === 'cancelled' ? (
                        <div className="p-3 bg-rose-50 border border-rose-100 text-rose-700 text-xs rounded-xl flex items-start gap-2 mt-2">
                          <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                          <div>
                            <span className="font-bold">Cancellation Details:</span> {event.notes || 'No reason provided.'}
                          </div>
                        </div>
                      ) : (
                        event.notes && (
                          <div className="p-3 bg-slate-50 border border-slate-150 text-slate-600 text-[11px] rounded-xl mt-2 leading-relaxed">
                            <span className="font-bold text-slate-700">Notes:</span> {event.notes}
                          </div>
                        )
                      )}
                    </div>
